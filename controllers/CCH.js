// const customerCommunication = require("../data/CCH")
...
const combArray = (req,res,next) =>{

    customerCommunication.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); 
 
    //console.log(customerCommunication)
    
    let idArray = []
    
    for(c of callRecord){
      idArray.push(Number(c.id))
    }
    
    let latestCustomerCommunication = []
    customerCommunication.forEach((e)=>{
      if(idArray.indexOf(e.userId) != -1){
        console.log(idArray)
        latestCustomerCommunication.push(e)
        idArray.splice(idArray.indexOf(e.userId),1)
      }
    })    
    
    latestCustomerCommunication.sort((a, b) => a.userId - b.userId);
    
    for(let i=0;i<latestCustomerCommunication.length;i++){
      callRecord[i].type = latestCustomerCommunication[i].type
    }
   
const combinedArray = [...customerCommunication, ...callRecord]
